classdef ProspectiveSchedulerApp < matlab.apps.AppBase
    ...
    methods (Access = public)
        ...
        function renderProposedSchedule(app)
            ...
            % DEBUG: dump first case timing when in Proposed for status investigation (remove after fix)
            try
                labs = annotatedSchedule.labAssignments();
                if ~isempty(labs)
                    for li = 1:numel(labs)
                        if isempty(labs{li}), continue; end
                        c = labs{li}(1);
                        fprintf('[DEBUG][renderProposed] lab=%d case=%s start=%.2f procStart=%.2f procEnd=%.2f post=%.2f turn=%.2f now=%.2f\n', ...
                            li, conduction.utils.conversion.asString(c.caseID), ...
                            conduction.gui.controllers.ScheduleRenderer.getFieldValue(c, {'startTime','scheduleStartTime'}, NaN), ...
                            conduction.gui.controllers.ScheduleRenderer.getFieldValue(c, {'procStartTime','startTime'}, NaN), ...
                            conduction.gui.controllers.ScheduleRenderer.getFieldValue(c, {'procEndTime','endTime'}, NaN), ...
                            conduction.gui.controllers.ScheduleRenderer.getFieldValue(c, 'postTime', 0), ...
                            conduction.gui.controllers.ScheduleRenderer.getFieldValue(c, {'turnoverTime','turnoverMinutes'}, 0), ...
                            nowMinutes);
                        break;
                    end
                end
